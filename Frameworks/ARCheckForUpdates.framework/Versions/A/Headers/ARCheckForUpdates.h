...
#define ARUpdateManagerUpdateDetectedNotification @"ARUpdateManagerUpdateDetectedNotification"
...
@class ARUpdateInfo, ARUpdate, AZUnzipper;
...
@interface ARUpdateManager : NSWindowController {
	...
	AZUnzipper *mUnzipper;
	NSString *mTemporaryPath;
	...
	BOOL mIsSoftwareUpdate;
	IBOutlet NSWindow *mSoftwareUpdateWindow;
	IBOutlet NSTextView *mSoftwareUpdateDescriptionTextView;
    
	NSURL *mInstallErrorURL;
...
-(void)setIsSoftwareUpdate:(BOOL)inSoftwareUpdate;
-(void)setInstallErrorURL:(NSURL *)inManualDownloadURL;
...
+(void)dontAutoCheckAtStartup;

-(void)blockAutoCheck;
-(void)releaseAutoCheck;
